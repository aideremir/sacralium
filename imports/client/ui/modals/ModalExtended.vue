...
transition(name="fade")
  .ModalExtended(v-if="modals.length")
    //- Тень всегда приходит одна, если все модалки обрабатываются в одном потоке
    .ModalExtended__shadow(@click="closeActiveModal")
    .ModalExtended__dialog(v-for="(modal, index) in modals")
      FrameComponent.ModalExtended__box
        .ModalExtended__head {{ modal.head }}
        .ModalExtended__text {{ modal.text }}

        .ModalExtended__inputWrapper(
          v-if="modal.promptValue !== undefined && modal.onAccept !== undefined"
        )
          BaseInput.ModalExtended__input(v-model="modal.promptValue")

        .ModalExtended__actions
          template(
            ...
            BaseButton(
              ...
              text="Принять"
              class='ModalExtended__action ModalExtended__action_accept'
            )
            BaseButton(
              @click="reject(index, modal.onReject)"
              light
              text="Отменить"
              class='ModalExtended__action ModalExtended__action_reject'
            )
          template(v-else)
            BaseButton(
              v-if="!modal.preventClose"
              @click="removeModal(index)"
              text="OK"
            ).ModalExtended__action
...
export default {
  ...
  mounted() {
    ...
    this.$root.$on(
      'updateModalExtended',
      ({ id, modal }) => this.updateModalById(id, modal),
    );
    this.$root.$on(
      'deleteModalExtended',
      ({ id, params }) => this.deleteModalById(id, params),
    );
  },
  methods: {
    addModal(obj) {
    ...
    updateModalById(id, modal) {
      const modalIdx = this.modals.findIndex(item => item.id === id);
      if (modalIdx === -1) {
        return;
      }
      const updatedModal = Object.assign(this.modals[modalIdx], modal);
      this.$set(this.modals, modalIdx, updatedModal);
    },
    deleteModalById(id, params = {}) {
      const modalIdx = this.modals.findIndex(item => item.id === id);
      if (modalIdx === -1) {
        return;
      }
      if (params.processRejectCallback) {
        this.reject(modalIdx, this.modals[modalIdx].onReject);
        return;
      }
      this.removeModal(modalIdx);
    },
    removeModal(index) {
      this.modals.splice(index, 1);
    },
    closeActiveModal() {
      if (this.activeModal.preventClose) {
        return;
      }
...
## Окно диалога
```js
import ModalExtended from '/imports/client/ui/modals/ModalExtended';
```
#### Использование
...
`preventClose`:Boolean - запрещает закрытие модельного окна из интерфейса пользователя
